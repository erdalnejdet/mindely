...
const COPY = {
  ...
  heroSubtitle: "Hesabınıza giriş yaparak randevularınızı yönetin.",
  registerPrompt: "Hesabınız yok mu? ",
  registerLinkText: "Kayıt olun",
  registerHref: "/auth/register",
} as const;

export function LoginForm({ showMarketingColumn = true }: { showMarketingColumn?: boolean }) {
    ...
    <>
      ...
      <div className="mx-auto w-full max-w-sm lg:max-w-md">
        <div className="mt-12">
          ...
          <p className="mt-2 text-muted-foreground">{COPY.heroSubtitle}</p>
        </div>
        ...
        <p className="mt-8 text-center text-sm text-muted-foreground">
            {COPY.registerPrompt}
            <Link href={COPY.registerHref} className="font-medium text-primary hover:underline">
  ...
  return (
    <div className="min-h-screen flex">
      <div className="flex flex-1 flex-col justify-center px-6 py-12 lg:px-20 xl:px-24">{inner}</div>
        ...
        <div className="relative flex h-full flex-col justify-between p-12">
          <div className="rounded-2xl bg-white/10 p-6 backdrop-blur">
            <p className="text-lg text-white">
              &ldquo;Mindely sayesinde kendime uygun psikoloğu buldum ve ilk seanstan itibaren farkı
              hissettim.&rdquo;
            </p>
            <p className="mt-4 text-sm text-emerald-100">— Platform kullanıcısı</p>
          </div>
